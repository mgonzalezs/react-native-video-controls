...
export default class VideoPlayer extends Component {
  ...
  hideControlAnimation() {
    Animated.parallel([
      ...
      Animated.timing(this.animations.playPause.opacity, {
        toValue: 0,
        duration: this.props.controlAnimationTiming,
        useNativeDriver: false,
      }),
   ...
   */
  renderBack() {
    if (this.props.renderBackControl) {
      return this.props.renderBackControl();
    }
   ...
   */
  renderPlayPause() {
    const { loading, error, showControls } = this.state;
    if (loading || error) {
      return null;
    }
    ...
    return (
      <Animated.View
        style={[
          styles.controls.playPauseControlsWrapper,
          {
            opacity: this.animations.playPause.opacity,
          },
        ]}
      >
        <View style={styles.controls.playPauseControls}>
          {showControls ? content : null}
        </View>
      </Animated.View>
    );
  }
